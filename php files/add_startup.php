<?php 
    ...
    $sql = "SELECT Mid,Mname FROM mentor WHERE Memail='$mentorname';";
    ...
    $sql = "SELECT Did FROM mentordomain WHERE Mid='$mid';";
    $results = $conn->query($sql);
    if (!empty($results) && $results->num_rows > 0) 
    {
        while ($row = $results->fetch_assoc()) 
        {
              array_push($did,$row["Did"]);
        }
    }
    // print_r($did);
...
<head>
    ...
    <link rel="stylesheet" href="css/add_startup.css">
...
-->
      ...
      <ul class="nav justify-content-end" id="rightmenu">
         <?php 
           ...
           echo '<a class="nav-link animate__animated animate__zoomIn" href="login.html"><i class="fa fa-user" aria-hidden="true"></i> Login</a>';
           echo '</li>';
           echo '<li class="nav-item">';
           echo '<a class="nav-link animate__animated animate__zoomIn" href="signup.html"><i class="fa fa-user-plus" aria-hidden="true"></i> Sign Up</a>';
           echo '</li>';
    ...
    <div class="title">
       ...
       <?php 
          echo '<strong><p id="heading">NEW STARTUPS</p></strong>';
          echo '<strong><p id="mentor-name"><i class="fas fa-user-circle"></i>Welcome '.$mname.'</p></strong>';
        ?>
       <a href="mentor.php" id="add-button">Mentor's Desk</a>
    </div>
    ...
    <?php 
         ...
         { 
              $dtemp = $did[$i];
              // echo $dtemp;
              $sql = "SELECT Tname,Tproject,Tstatus FROM team WHERE Did='$dtemp';";
          $results = $conn->query($sql);
          if (!empty($results) && $results->num_rows > 0) 
          {
    ...
    <div class="main-body">
      ...
      <div class="accordion" id="accordionExample">
         <?php 
              ...
              for ($i=0; $i < $tmlen; $i++) 
              { 
                  echo '<form action="add_startup.php" method="POST">';
                  echo '<div class="card">';
                  ...
                  echo $tname[$i];
                  echo '</button>';
                  echo '<p visibility="visible">';
                  echo '<input type="text" name="input1" id="input1" placeholder="Re-enter the team name">';
                  echo '</p>';
                  echo '</h2>';
                  echo '<button type="button" name="button" class="delete-button">';
                  echo '<i class="fas fa-trash-alt"></i></button>';
                  echo '<button type="submit" name="submit" class="as">+Add Startup</button>';
                  echo '</div>';
              ...
              {
                $_SESSION["mid"] = $mid; 
                $_SESSION["tname"] = $_POST["input1"];
              echo '<script type="text/javascript">';  
                        echo 'window.location.href = "padd_startup.php";';
                        echo '</script>';
              }
          ?>
